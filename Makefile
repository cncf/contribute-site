...
get-link-checker:
	...
	@if ! command -v go >/dev/null 2>&1; then \
		...
		echo "Alternative: Install a pre-built binary from https://github.com/wjdp/htmltest/releases"; \
		echo "             or use the bash install script: curl https://htmltest.wjdp.uk | bash"; \
		exit 1; \
	fi
